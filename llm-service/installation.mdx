...
Ensure your server meets the minimum requirements and that you can run Docker with GPU access. See System Requirements in the LLM Service overview.
</Callout>
...
<Callout type="info" title="Air-gapped?">
If your environment is air-gapped, pre-load the container images and model weights, then run the installer with the offline flag provided by your Visibl representative.
</Callout>
...
<Callout type="success" title="Model selection guidance">
Start with the Fast model for most workflows. Switch to Heavy when you need deeper multi-step reasoning or long-context analysis.
</Callout>
...
Increasing utilization may improve throughput but can reduce headroom and stability on memory-constrained GPUs. Adjust gradually.
</Callout>
...
<Callout type="info" title="API quick test">
The service exposes an OpenAI-compatible endpoint. Try a short request with your preferred SDK to confirm connectivity.
</Callout>
