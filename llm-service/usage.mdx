...
client = openai.OpenAI(
    base_url="http://127.0.0.1:58180/v1",
    api_key="visibl-local-key"  # Any string works
)

response = client.chat.completions.create(
    model="visibl-70b",
    messages=[
        {"role": "user", "content": "Review this Verilog module for timing issues"}
    ],
    temperature=0.1
)
...
curl -X POST http://127.0.0.1:58180/v1/chat/completions \
  ...
  -d '{
    "model": "visibl-70b",
    "messages": [{"role": "user", "content": "Explain FIFO design principles"}],
    "stream": true
  }'
...
curl -X POST http://127.0.0.1:58180/v1/chat/completions \
  ...
  -d '{
    "model": "visibl-70b",
    "messages": [
      {"role": "system", "content": "You are an expert RTL design engineer."},
      {"role": "user", "content": "Help me debug this clock domain crossing issue"}
    ]
  }'
```
